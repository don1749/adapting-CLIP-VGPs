...
WORK_DIR = '/work/adapting-CLIP-VGPs/'
...
def load_dataset(rank, batch_size, args):
    ...
    text_only = True if args.model=='TextOnly' else False
    train_dataset = VGPsHeatmapsDataset(split='train', text_only=text_only)
    val_dataset = VGPsHeatmapsDataset(split='val', text_only=text_only)
...
def setup_model(gpu, args):
    ...
    if args.model == 'TextOnly':
        net = VgpClassifier(text_only=True).to(gpu)
    elif args.model == 'MapOnly':
        net = VgpClassifier(heatmap_only=True).to(gpu)
    else:
        net = VgpClassifier(text_only=False, heatmap_only=False).to(gpu)

    optimizer = optim.SGD(net.parameters(), lr=args.lr, momentum=0.9)

    if args.checkpoint != 0:
        checkpoint_path = f'{WORK_DIR}/checkpoints/checkpoint{args.checkpoint}.pt'
    ...
    model = DDP(net, device_ids=[gpu], find_unused_parameters=True)
...
def train(gpu, args):
    ...
    history_file_path = f'{WORK_DIR}/checkpoints/training_history.json'
    ...
    for epoch in range(args.checkpoint, args.epochs):
        for phase in ['train', 'val']:
            ...
            for batch in tqdm(dataloader):
                ...
                left_heatmaps = None if args.model=='TextOnly' else batch['left_heatmap']
                right_heatmaps = None if args.model=='TextOnly' else batch['right_heatmap']
                labels = batch['label']
        ...
        torch.save({'epoch': epoch,
                   ...
                   '{WORK_DIR}/checkpoints/checkpoint{}.pt'.format(epoch + 1)) 
...
def main():
    ...
    parser.add_argument('--num_samples', type=int, default=0)  # 0 to test all samples
