...
def setup_model(gpu, args):
    ...
    if args.model == 'CNN':
        ...
        sim_net = TextHeatmapGatedClassifier(heatmap_only=False).to(gpu)
    elif args.model == 'TextHeatmapSoftmax':
        sim_net = TextHeatmapSoftmaxClassifier(heatmap_only=False, gating=True, use_dropout=True).to(gpu)
    elif args.model == 'TextHeatmapSigmoid':
        sim_net = TextHeatmapSigmoidClassifier().to(gpu)
    elif args.model == 'TextHeatmapNoCNNSigmoid':
        sim_net = TextHeatmapNoCNN().to(gpu)
    elif args.model == 'SimplifiedTextHeatmapSigmoid':
        sim_net = SimplifiedTextHeatmapGatedClassifier(heatmap_only=False).to(gpu)
    elif args.model == 'TextHeatmapNoCNNSubSigmoid':
        sim_net = TextHeatmapNoCNNSubtraction().to(gpu)
    elif args.model == 'TextHeatmapSubCNNSigmoid':
        sim_net = TextHeatmapFinal().to(gpu)
    else:
        assert False
...
def train(gpu, args):
    ...
    history_file_path = '/work/adapting-CLIP-VGPs/checkpoints/text+heatmap2/{}_training_history.json'.format(args.expno)
    ...
    for epoch in range(args.checkpoint, args.epochs):
        ...
        torch.save({'epoch': epoch,
                   ...
                   '/work/adapting-CLIP-VGPs/checkpoints/text+heatmap2/{}_checkpoint{}.pt'.format(args.expno, epoch + 1)) 
