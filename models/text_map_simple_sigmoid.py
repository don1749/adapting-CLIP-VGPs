...
class SimplifiedTextHeatmapGatedClassifier(nn.Module):
    def __init__(self, heatmap_only=False, text_only=False, use_dropout=False):
        super(SimplifiedTextHeatmapGatedClassifier, self).__init__()
        # 
    ...
    def forward(self, x1, x2, t1, t2):
        ...
        if self.text_only:
            logits = self.logits(self.relu(h_t))
            return logits
        ...
        if self.heatmap_only:
            logits = self.logits(self.relu(h_v))
            return logits
        
        # fuse visual and text with gates
        visual_gate, text_gate = self.gate_calc(v,t)
        tanh = nn.Tanh()
        y = visual_gate*tanh(h_v) + text_gate*tanh(h_t)
