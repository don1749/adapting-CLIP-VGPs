...
from models.text_hm_2layerCNN import TextHeatmapCNN
from models.clip_text_heatmap import TextHeatmapGatedClassifier
...
def load_dataset(rank, batch_size, args):
    ...
    train_dataset = VGPsHeatmapsDataset(split='train', text_only=True)
    val_dataset = VGPsHeatmapsDataset(split='val', text_only=True)
...
def setup_model(gpu, args):
    ...
    if args.model == 'CNN':
        sim_net = TextHeatmapCNN().to(gpu)
    elif args.model == 'TextHeatmapGated':
        sim_net = TextHeatmapGatedClassifier(heatmap_only=False, text_only=True).to(gpu)
    elif args.model == 'TextHeatmapSoftmax':
        sim_net = TextHeatmapSoftmaxClassifier(heatmap_only=False, text_only=True, use_dropout=True).to(gpu)
    elif args.model == 'TextHeatmapNoCNNSigmoid':
        sim_net = TextHeatmapNoCNN(text_only=True).to(gpu)
    elif args.model == 'TextHeatmapSigmoid':
        sim_net = TextHeatmapSigmoidClassifier().to(gpu)
    elif args.model == 'TextHeatmapNoCNNSubSigmoid':
        sim_net = TextHeatmapNoCNNSubtraction(text_only=True).to(gpu)
    elif args.model == 'TextOnlySigmoid':
        sim_net = TextClassifier().to(gpu)
    elif args.model == 'TextHeatmapSubCNNSigmoid':
        sim_net = TextHeatmapFinal(text_only=True).to(gpu)
    else:
        assert False
    ...
    if args.checkpoint != 0:
        checkpoint_path = f'/work/adapting-CLIP-VGPs/checkpoints/text only/{args.expno}_checkpoint{args.checkpoint}.pt'
...
def train(gpu, args):
    ...
    history_file_path = '/work/adapting-CLIP-VGPs/checkpoints/text only/{}_training_history.json'.format(args.expno)
    ...
    if args.model == 'TextHeatmapSoftmax':
    ...
    for epoch in range(args.checkpoint, args.epochs):
        for phase in ['train', 'val']:
            ...
            for batch in tqdm(dataloader):
                ...
                with torch.set_grad_enabled(phase=='train'):
                    outputs = model(None, None, left_text_ft, right_text_ft)
        ...
        torch.save({'epoch': epoch,
                   ...
                   '/work/adapting-CLIP-VGPs/checkpoints/text only/{}_checkpoint{}.pt'.format(args.expno, epoch + 1)) 
         # Save the training history after each epoch
        with open('/work/adapting-CLIP-VGPs/checkpoints/text only/{}_training_history.json'.format(args.expno), 'w') as f:
