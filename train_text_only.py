...
class TextEmbClassifier(nn.Module):
    def __init__(self):
        ...
        self.textliner = nn.Sequential(nn.Linear(768, 4096), nn.Sigmoid())
        self.out = nn.Linear(4096, 1, dtype=torch.float16)
        # weight init
...
def setup_model(gpu, args):
    ...
    if args.model == 'CNN':
        sim_net = TextEmbClassifier()
    else:
        assert False

    if args.checkpoint != 0:
        checkpoint_path = f'/work/adapting-CLIP-VGPs/checkpoints/text_only/checkpoint{args.checkpoint}.pt'
...
def train(gpu, args):
    ...
    for epoch in range(args.checkpoint, args.epochs):
        ...
        torch.save({'epoch': epoch,
                   ...
                   '/work/adapting-CLIP-VGPs/checkpoints/text_only/checkpoint{}.pt'.format(epoch + 1)) 
         # Save the training history after each epoch
        with open('/work/adapting-CLIP-VGPs/checkpoints/text_only/training_history.json', 'w') as f:
